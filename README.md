...
6. به‌عنوان مشتری توضیح سفارش رو بفرست؛ بعد به‌عنوان ادمین `/orders` رو بزن و با دکمه‌ی ✅ تأیید کن. باید پیام تأیید به مشتری برسه.
7. `/myorders` رو بزن (هم به‌عنوان مشتری) تا ببینی سفارش با وضعیت «نهایی‌شده» نشون داده می‌شه.

### دستورات ادمین در بات
| دستور | کار |
|---|---|
| `/add` | افزودن محصول جدید (فلوی مرحله‌ای) |
| `/list` | لیست محصولات با دکمه‌های ویرایش/حذف/کم‌کردن موجودی |
| `/orders` | بررسی سفارش‌های در انتظار، با دکمه‌ی تأیید نهایی یا رد |
| `/stats` | آمار فروشگاه و سفارش‌ها (به تفکیک وضعیت) |
| `/shop` | باز کردن فروشگاه به‌صورت Mini App (برای تست) |
| `/help` | راهنما |

### دستورات مشتری در بات
| دستور | کار |
|---|---|
| `/start` | شروع / باز کردن فروشگاه |
| `/shop` | باز کردن فروشگاه به‌صورت Mini App |
| `/myorders` | تاریخچه‌ی کامل سفارش‌های خودش (نهایی‌شده، در‌انتظار، نصفه‌نیمه، منصرف‌شده) |
...
### 🧾 سیستم سفارش و وضعیت‌ها
موجودی محصول **هیچ‌وقت خودکار کم نمی‌شه** — نه وقتی مشتری سفارش می‌ده، نه وقتی ادمین تأیید می‌کنه. این عمدی‌ست، چون خیلی از سفارش‌ها نهایی نمی‌شن (مشتری منصرف می‌شه یا جواب نمی‌ده) و کم کردن خودکار باعث می‌شه موجودی واقعی غلط بشه. کم کردن موجودی همیشه با **اقدام صریح ادمین** انجام می‌شه: از `/list`، دکمه‌ی «📦 کم کردن موجودی» رو بزن و یا «۱ تا کم کن» یا یک عدد دلخواه وارد کن.

هر سفارش یکی از این چهار وضعیت رو داره (در فایل محلی `bot/orders.json`، که هیچ‌وقت روی گیت‌هاب پوش نمی‌شه چون دیتای مشتری نباید عمومی باشه):

| وضعیت | یعنی چی |
|---|---|
| 🟡 `incomplete` (نصفه‌نیمه) | مشتری روی «سفارش» زده ولی هنوز توضیح/تماس نفرستاده |
| 🔵 `pending` (در انتظار) | مشتری توضیح فرستاده، منتظر تأیید یا رد ادمینه |
| ✅ `confirmed` (نهایی‌شده) | ادمین از `/orders` تأیید کرده |
| 🚫 `cancelled` (منصرف/رد‌شده) | مشتری خودش با دکمه‌ی «انصراف» لغو کرده، ادمین رد کرده، یا بعد از ۳۰ دقیقه بی‌پاسخی خودکار منقضی شده |

یک ترد پشت‌زمینه‌ی جدا، هر ۵ دقیقه سفارش‌های `incomplete`/`pending` قدیمی‌تر از ۳۰ دقیقه رو خودکار `cancelled` می‌کنه (قابل تغییر با `ABANDON_TIMEOUT_MINUTES` در `orders_store.py`).

مشتری با `/myorders` تاریخچه‌ی کامل خودش رو می‌بینه؛ ادمین با `/orders` فقط سفارش‌های در‌انتظار رو با دکمه‌ی تأیید/رد می‌بینه. `/stats` هم تعداد هر وضعیت رو نشون می‌ده.
...
- مدل فعلی خرید همچنان «هماهنگی دستی»ست: مشتری از سایت/Mini App سفارش رو شروع می‌کنه، ادمین با `/orders` تأیید یا رد می‌کنه، و کم کردن موجودی یک قدم کاملاً جدا و دستیه.
- تقریباً همه‌ی اقدام‌ها (تأیید/رد سفارش، انصراف مشتری، ویرایش/حذف محصول، کم کردن موجودی، افزودن محصول بعدی) از طریق دکمه‌ی شیشه‌ای (inline keyboard) انجام می‌شن، نه تایپ دستی دستور.
...
6. As the customer, send order details; then as the admin, run `/orders` and tap ✅ to confirm. The customer should get a confirmation message.
7. Run `/myorders` as the customer to see the order listed with "confirmed" status.

### Bot admin commands
| Command | Action |
|---|---|
| `/add` | Add a new product (step-by-step flow) |
| `/list` | List products with edit/delete/decrease-stock buttons |
| `/orders` | Review pending orders, with confirm/reject buttons |
| `/stats` | Shop and order stats (broken down by status) |
...
| Command | Action |
|---|---|
| `/start` | Start / open the shop |
| `/shop` | Open the shop as a Mini App |
| `/myorders` | Full order history (confirmed, pending, incomplete, cancelled) |
...
Stock is **never decremented automatically** — not when a customer orders, not even when an admin confirms it. This is intentional: many orders never get finalized (customers back out or go silent), and auto-decrementing would make stock counts inaccurate. Decreasing stock is always an **explicit admin action**: from `/list`, tap "📦 Decrease stock" and either "−1" or enter a custom amount.

Every order has one of four statuses (stored locally in `bot/orders.json`, never pushed to GitHub since customer data shouldn't be public):

| Status | Meaning |
|---|---|
| 🟡 `incomplete` | Customer tapped "order" but hasn't sent contact details yet |
| 🔵 `pending` | Customer sent details, waiting on admin confirm/reject |
| ✅ `confirmed` | Admin confirmed it via `/orders` |
| 🚫 `cancelled` | Customer cancelled themselves, admin rejected it, or it auto-expired after 30 minutes of silence |

A separate background thread checks every 5 minutes and auto-cancels `incomplete`/`pending` orders older than 30 minutes (configurable via `ABANDON_TIMEOUT_MINUTES` in `orders_store.py`).

Customers see their full history via `/myorders`; admins see only pending orders (with confirm/reject buttons) via `/orders`. `/stats` also breaks down counts by status.
...
- The ordering model is still "manual coordination": the customer starts an order from the site/Mini App, the admin confirms or rejects it via `/orders`, and decreasing stock is a fully separate, manual step.
- Nearly every action (confirm/reject order, customer cancel, edit/delete product, decrease stock, add another product) happens through inline buttons rather than typed commands.
